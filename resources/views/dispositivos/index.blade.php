                        ...
                        <table class="table align-items-center mb-0">
                            ...
                            <img src="{{asset('img/team-2.jpg')}}" class="avatar avatar-sm me-3 border-radius-lg" alt="user1">
                                                ...
                                                <div class="d-flex flex-column justify-content-center">
                                                    <h6 class="mb-0 text-sm">{{ $dispositivo->nombre_dispositivo }}</h6>
                                                    {{-- <p class="text-xs text-secondary mb-0">john@creative-tim.com</p> --}}
                                                </div>
                                            </div>
                                        </td>
                                        <td class="align-middle text-sm">
                                            <p class="text-xs font-weight-bold mb-0 ">{{ $dispositivo->codigo }}</p>
                                            {{-- <p class="text-xs text-secondary mb-0">Organization</p> --}}
                                        </td>
                                        <td class="align-middle text-sm">
                                            <span
                                                class="text-xs font-weight-bold mb-0 ">{{ $dispositivo->descripcion }}</span>
                                        </td>
                                        <td class="align-middle text-center">
                                            <input type="checkbox" {{ $dispositivo->estado == 'habilitado' ? 'checked' : '' }} disabled>
                                        </td>
                                        <td class="align-middle">
                                          <a href="{{url('/dispositivos/'.$dispositivo->id.'/edit')}}" class="text-warning font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit user">
                                            <span class="alert-icon align-middle">
                                              <span class="material-icons text-md">
                                                edit
                                              </span>
                                          ...
                                          <form action="{{ URL('/dispositivos/'.$dispositivo->id) }}" method="POST" id="eliminarRegistroForm" style="display: none;">
                                              @csrf
                                              @method('DELETE')
                                          </form>
